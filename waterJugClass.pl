...
move([X,Y],'Fill 4-gallon jug from 3-gallon jug',[4, Y1]) :- Y1 is (X+Y)-4, X+Y>=4, X>=0, Y>0.
%
% 6. Rule corresponding to the move 'Fill 3-gallon jug from 4-gallon
% jug'
move([X,Y],'Fill 3-gallon jug from 4-gallon jug',[X1, 3]) :- X1 is (X+Y)-3, X+Y>=3, X>0, Y>=0.
%
%7. Rule corresponding to the move 'Empty 3-gallon jug into 4-gallon jug'
move([X,Y],'Empty 3-gallon jug into 4-gallon jug',[X1, 0]) :- X1 is X+Y, X1=<4,Y>0, X>=0.
%
%8. Rule corresponding to the move 'Empty 4-gallon jug into 3-gallon jug'
move([X,Y],'Empty 4-gallon jug into 3-gallon jug',[0,Y1]) :- Y1 is X+Y, Y1=<3, X>0, Y>=0.
